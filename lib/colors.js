...
var pafiumeColors = function () {
  ...
  var randomColorIndex = function randomColorIndex() {
    return Math.floor(Math.random() * colorNames.length);
  };

  var doesNotExistInScheme = function doesNotExistInScheme(scheme, color) {
    var name = color.name;
    var scheme = scheme.map(function (color) {
      return color.name;
    });
    return !scheme.includes(name);
  };

  var createColorObject = function createColorObject(name, hues) {
    return { name: name, hues: hues };
  };
